...
public class DashboardP4Ponto1Ponto2Controller {
    ...
    private final String pastaExcels = System.getProperty("user.home") + File.separator + "FeiraVivaData" + File.separator + "P4";
    private final String caminhoLocal = pastaExcels + File.separator + "P4.1.2 Avaliação Professores.xlsx";
    ...
    private void onLoadExcel() {
        try {
            ...
            workbook = ExcelFileDAO.getExcelFromDB("P4.1.2 Avaliação Professores.xlsx");
            if (workbook == null) {
                // Se não houver na BD, importa dos resources e salva na BD para futura utilização
                try (InputStream is = getClass().getResourceAsStream("/com/ExcelFiles/P4/P4.1.2 Avaliação Professores.xlsx")) {
                    if (is == null) {
                        System.out.println("❌ Ficheiro inicial não encontrado nos resources!");
                        return;
                    }
                    workbook = new XSSFWorkbook(is);
                    ExcelFileDAO.saveExcelToDB("P4.1.2 Avaliação Professores.xlsx", utilizador.getId(), workbook);
        ...
        try {
            ...
            if (file.exists() && file.lastModified() > ultimaEdicaoTabela) {
                ...
                try (FileInputStream fis = new FileInputStream(file)) {
                    ...
                    ExcelFileDAO.saveExcelToDB("P4.1.2 Avaliação Professores.xlsx", utilizador.getId(), editedWb);
                    workbook = editedWb;
                    sheet = workbook.getSheetAt(0);
                    evaluator = workbook.getCreationHelper().createFormulaEvaluator();
    ...
    private void loadExcelFile() {
        try {
            ...
            for (int i = 0; i < colCount; i++) {
                ...
                Cell headerCell = headerRow.getCell(i); // NOVO
                String colName = (headerCell != null) ? headerCell.getStringCellValue() : ""; // NOVO
                TableColumn<ObservableList<String>, String> col = new TableColumn<>(colName); // ALTERADO
    ...
    private void onExcelButtonClick() {
        try {
            ...
            XSSFWorkbook wb = ExcelFileDAO.getExcelFromDB("P4.1.2 Avaliação Professores.xlsx");
            if (wb != null) {
    ...
    private void onBackLabelClick() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/fxml/feiraviva/DashboardP4Geral.fxml"));
            Parent root = loader.load();

            Object controller = loader.getController();
            if (controller instanceof DashboardP4GeralController) {
