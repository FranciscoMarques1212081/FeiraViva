...
public class DashboardP11Ponto2Controller {
    @FXML
    ...
    private final String pastaExcels = System.getProperty("user.home") + File.separator + "FeiraVivaData" + File.separator + "P11";
    private final String caminhoLocal = pastaExcels + File.separator + "2º Contrato Programa.xlsx";
    ...
    private void onLoadExcel() {
        try {
            ...
            workbook = ExcelFileDAO.getExcelFromDB("2º Contrato Programa.xlsx");
            if (workbook == null) {
                // Se não houver na BD, importa dos resources e salva na BD para futura utilização
                try (InputStream is = getClass().getResourceAsStream("/com/ExcelFiles/P11/2º Contrato Programa.xlsx")) {
                    if (is == null) {
                        System.out.println("❌ Ficheiro inicial não encontrado nos resources!");
                        return;
                    }
                    workbook = new XSSFWorkbook(is);
                    ExcelFileDAO.saveExcelToDB("2º Contrato Programa.xlsx", utilizador.getId(), workbook);
        ...
        try {
            ...
            if (file.exists() && file.lastModified() > ultimaEdicaoTabela) {
                ...
                try (FileInputStream fis = new FileInputStream(file)) {
                    ...
                    ExcelFileDAO.saveExcelToDB("2º Contrato Programa.xlsx", utilizador.getId(), editedWb);
                    workbook = editedWb;
                    sheet = workbook.getSheetAt(0);
                    evaluator = workbook.getCreationHelper().createFormulaEvaluator();
    ...
    private void loadExcelFile() {
        try {
            ...
            boolean hasHeader = false; // Declarado fora de blocos
            int colCount = 0;
            ...
            if (!hasHeader) {
                // Sem header: tenta encontrar primeira linha com dados
                Row firstDataRow = null;
                int rowNum = 0;
                while ((firstDataRow == null) && (rowNum <= sheet.getLastRowNum())) {
                    firstDataRow = sheet.getRow(rowNum++);
                }
            ...
            // Passa para final para uso na lambda
    ...
    private void onExcelButtonClick() {
        try {
            ...
            XSSFWorkbook wb = ExcelFileDAO.getExcelFromDB("2º Contrato Programa.xlsx");
            if (wb != null) {
    ...
    private void onBackLabelClick() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/fxml/feiraviva/DashboardP11Geral.fxml"));
            Parent root = loader.load();

            Object controller = loader.getController();
            if (controller instanceof DashboardP11GeralController) {
