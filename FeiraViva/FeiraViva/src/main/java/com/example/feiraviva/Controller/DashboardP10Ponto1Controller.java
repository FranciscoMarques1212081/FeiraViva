...
public class DashboardP10Ponto1Controller {
    @FXML
    ...
    private final String pastaExcels = System.getProperty("user.home") + File.separator + "FeiraVivaData" + File.separator + "P10";
    private final String caminhoLocal = pastaExcels + File.separator + "P10.1 AjusteDireto e Consulta Prévia.xlsx";
    ...
    private void onLoadExcel() {
        try {
            ...
            workbook = ExcelFileDAO.getExcelFromDB("P10.1 AjusteDireto e Consulta Prévia.xlsx");
            if (workbook == null) {
                // Se não houver na BD, importa dos resources e salva na BD para futura utilização
                try (InputStream is = getClass().getResourceAsStream("/com/ExcelFiles/P10/P10.1 AjusteDireto e Consulta Prévia.xlsx")) {
                    if (is == null) {
                        System.out.println("❌ Ficheiro inicial não encontrado nos resources!");
                        return;
                    }
                    workbook = new XSSFWorkbook(is);
                    ExcelFileDAO.saveExcelToDB("P10.1 AjusteDireto e Consulta Prévia.xlsx", utilizador.getId(), workbook);
        ...
        try {
            ...
            if (file.exists() && file.lastModified() > ultimaEdicaoTabela) {
                ...
                try (FileInputStream fis = new FileInputStream(file)) {
                    ...
                    ExcelFileDAO.saveExcelToDB("P10.1 AjusteDireto e Consulta Prévia.xlsx", utilizador.getId(), editedWb);
                    workbook = editedWb;
                    sheet = workbook.getSheetAt(0);
                    evaluator = workbook.getCreationHelper().createFormulaEvaluator();
    ...
    private void loadExcelFile() {
        try {
            ...
            for (int i = 0; i < colCount; i++) {
                ...
                TableColumn<ObservableList<String>, String> col = new TableColumn<>(headerRow.getCell(i).getStringCellValue());
    ...
    private void onExcelButtonClick() {
        try {
            ...
            XSSFWorkbook wb = ExcelFileDAO.getExcelFromDB("P10.1 AjusteDireto e Consulta Prévia.xlsx");
            if (wb != null) {
    ...
    private void onBackLabelClick() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/fxml/feiraviva/DashboardP10Geral.fxml"));
            Parent root = loader.load();

            Object controller = loader.getController();
            if (controller instanceof DashboardP10GeralController) {
