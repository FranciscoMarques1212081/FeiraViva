...
public class DashboardP9Ponto1Ponto2Controller {
    @FXML
    ...
    private final String caminhoLocal = pastaExcels + File.separator + "P9.1.2 Satisfação dos clientes.xlsx";
    ...
    private void onLoadExcel() {
        try {
            ...
            workbook = ExcelFileDAO.getExcelFromDB("P9.1.2 Satisfação dos clientes.xlsx");
            if (workbook == null) {
                // Se não houver na BD, importa dos resources e salva na BD para futura utilização
                try (InputStream is = getClass().getResourceAsStream("/com/ExcelFiles/P9/P9.1.2 Satisfação dos clientes.xlsx")) {
                    if (is == null) {
                        System.out.println("❌ Ficheiro inicial não encontrado nos resources!");
                        return;
                    }
                    workbook = new XSSFWorkbook(is);
                    ExcelFileDAO.saveExcelToDB("P9.1.2 Satisfação dos clientes.xlsx", utilizador.getId(), workbook);
        ...
        try {
            ...
            if (file.exists() && file.lastModified() > ultimaEdicaoTabela) {
                ...
                try (FileInputStream fis = new FileInputStream(file)) {
                    ...
                    ExcelFileDAO.saveExcelToDB("P9.1.2 Satisfação dos clientes.xlsx", utilizador.getId(), editedWb);
                    workbook = editedWb;
                    sheet = workbook.getSheetAt(0);
                    evaluator = workbook.getCreationHelper().createFormulaEvaluator();
    ...
    private void onExcelButtonClick() {
        try {
            ...
            XSSFWorkbook wb = ExcelFileDAO.getExcelFromDB("P9.1.2 Satisfação dos clientes.xlsx");
            if (wb != null) {
