...
public class DashboardP1Ponto2Controller {
    ...
    private final String caminhoLocal = pastaExcels + File.separator + "P1.2 Receitas Próprias.xlsx";
    ...
    private void onLoadExcel() {
        try {
            ...
            workbook = ExcelFileDAO.getExcelFromDB("P1.2 Receitas Próprias.xlsx");
            if (workbook == null) {
                // Se não houver na BD, importa dos resources e salva na BD para futura utilização
                try (InputStream is = getClass().getResourceAsStream("/com/ExcelFiles/P1/P1.2 Receitas Próprias.xlsx")) {
                    if (is == null) {
                        System.out.println("❌ Ficheiro inicial não encontrado nos resources!");
                        return;
                    }
                    workbook = new XSSFWorkbook(is);
                    ExcelFileDAO.saveExcelToDB("P1.2 Receitas Próprias.xlsx", utilizador.getId(), workbook);
        ...
        try {
            ...
            if (file.exists() && file.lastModified() > ultimaEdicaoTabela) {
                ...
                try (FileInputStream fis = new FileInputStream(file)) {
                    ...
                    ExcelFileDAO.saveExcelToDB("P1.2 Receitas Próprias.xlsx", utilizador.getId(), editedWb);
                    workbook = editedWb;
                    sheet = workbook.getSheetAt(0);
                    evaluator = workbook.getCreationHelper().createFormulaEvaluator();
    ...
    private void onExcelButtonClick() {
        try {
            ...
            XSSFWorkbook wb = ExcelFileDAO.getExcelFromDB("P1.2 Receitas Próprias.xlsx");
            if (wb != null) {
